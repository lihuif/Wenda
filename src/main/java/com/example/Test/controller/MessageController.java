...
import com.example.Test.model.ViewObject;
    ...
    public String addMessage(@RequestParam("toName") String toName, @RequestParam("content") String content){
        try {
            if (hostHolder.getUser() == null){
                return TestUtil.getJSONString(999,"未登录");
            }
            User user = userService.selectByName(toName);
            if (user == null){
                return TestUtil.getJSONString(1, "用户不存在");
            }
            ...
            return TestUtil.getJSONString(1, "发送失败");
        }
    }
}
